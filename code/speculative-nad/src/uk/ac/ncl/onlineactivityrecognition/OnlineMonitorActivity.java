...
public class OnlineMonitorActivity extends Activity implements SensorEventListener {
	...
	final static int SWITCH_OVER_HOUR = 12;
	...
	final static int BUDGET_SIZE = 200;
			...
			if (slidingWindowEnabled && ((msg.what == MSG_NEW_READING_DEVICE_ONE) || 
				...
				if (frameOneCompleted && frameTwoCompleted && frameThreeCompleted && frameFourCompleted) {
					...
					try {
						...
						flushUnlabelledFrame(frame4d, t, smallConfidence, bigConfidence);
					} catch (Exception e) {
						throw new RuntimeException(e);
					}
					...
					try {
						...
						if (isInteractive) {
							...
							if (sbd.segmentBoundaryDetected()) {
								...
								final double finalAskProbNonNorm = askProbNonNorm;
								...
								runOnUiThread(new Runnable() {
									...
									public void run() {										
										...
										txvConfidence.setText("Confidence: " + finalConf + "%(" + 
												df.format(Math.max(finalAskProbNonNorm, finalAskProbNorm)) + ")");
								...
								//ask = true;
								//ask = false;
								Log.w("Annotation", "ask = " + ask + "(" + smallAsk + "|" + bigAsk + ")");
								++segmentCount;
	...
	Handler dbFlusherHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case MSG_FLUSH_LABELLED_INSTANCE_POJO:
				...
				Log.i("DB Flusher", "Saved labelled instance");
				break;
				/*
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		try {
			...
			if (requestCode == REQUEST_CODE_USER_ANNOTATION) {
				...
				for (int k = 0; k < CLASS_COVERAGE.length; k++) {
					if (resultCode == CLASS_COVERAGE[k]) {
						sb.append("*");
						sb.append(CLASS_COVERAGE[k]);
						sb.append("* ");
					} else {
						sb.append(CLASS_COVERAGE[k]);
						sb.append(" ");
				...
				Log.i("NB_cluster", "Class Coverage updated: " + sb.toString());
