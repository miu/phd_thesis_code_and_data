...
for (s.f in subjects.features) {
  ...
  if (s.f$task.number %in% test.task.numbers) {
    for (s.f.t.i in seq_along(subjects.features.test)) {
      ...
      if (s.f.t$subject.number == s.f$subject.number) {
        for (seg in s.f$data) {
          next.i = 1 + length(subjects.features.test[[s.f.t.i]]$data)
          subjects.features.test[[s.f.t.i]]$data[[next.i]] = seg
        }
        data.appended = T
        break
      }
  ...
  if (s.f$task.number %in% train.task.numbers) {
    for (s.f.t.i in seq_along(subjects.features.train)) {
      ...
      if (s.f.t$subject.number == s.f$subject.number) {
        for (seg in s.f$data) {
          next.i = 1 + length(subjects.features.train[[s.f.t.i]]$data)
          subjects.features.train[[s.f.t.i]]$data[[next.i]] = seg
        }
        data.appended = T
        break
      }
...
SaveObject(obj=subjects.features.test, 
           var.name='opportunity.subjects.dtw.test.250.incr',
           dir=opportunity.dir)

SaveObject(obj=subjects.features.train, 
           var.name='opportunity.subjects.dtw.train.250.incr',
           dir=opportunity.dir)
