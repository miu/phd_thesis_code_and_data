...
load('~/R/sf_ALTLAR/output/al.usc.had/pamap.2/l.al.frame.Rdata') ## <--- redone
...
load('~/R/sf_ALTLAR/output/al.usc.had/pamap.2/l.rs.frame.Rdata') ## <--- redone
...
#lines(y.plus.sd, col='red', lty='dotted')
#lines(x=1:length(y.minus.sd), y=rep(0, length(y.minus.sd)), col='black')
stop('before seg-based')


##### now seg-based annotation
m.al.full = NULL
m.rs.full = NULL
...
m.al.seg.myopic = NULL
m.rs.seg.myopic = NULL

for (subject.index in seq_len(num.subjects)) {
  al.perfs = GetPerformance.seg(l=l.al.seg, subject.index=subject.index)
  ...
  y.al.seg.myopic = al.perfs$perf.seg.myopic
  m.al.seg.myopic = rbind(m.al.seg.myopic, y.al.seg.myopic)
  
  y.al.seg.full = al.perfs$perf.seg.full
  m.al.seg.full = rbind(m.al.seg.full, y.al.seg.full)
  
  
  rs.perfs = GetPerformance.seg(l=l.rs.seg, subject.index=subject.index)
  ...
  y.rs.seg.myopic = rs.perfs$perf.seg.myopic
  m.rs.seg.myopic = rbind(m.rs.seg.myopic, y.rs.seg.myopic)
  
  y.rs.seg.full = rs.perfs$perf.seg.full
  m.rs.seg.full = rbind(m.rs.seg.full, y.rs.seg.full)
  ...
  #plot(y.al.seg.full, type='l', ylim=c(0, 1), col='red', main=paste('Full; Seg; Subject', subject.index))
  #lines(y.rs.seg.full, col='blue')
  
  #plot(y.al.seg.myopic, type='l', lty='dashed', ylim=c(0, 1), col='red', main=paste('Myopic; Seg; Subject', subject.index))
  #lines(y.rs.seg.myopic, lty='dashed', col='blue')
}

y.al.seg.full.mean = apply(X=m.al.seg.full, MARGIN=2, FUN=mean)
y.rs.seg.full.mean = apply(X=m.rs.seg.full, MARGIN=2, FUN=mean)
plot(y.al.seg.full.mean, type='l', lwd=2, ylim=c(0, 1), col='red', main="Full; Mean; Seg-based x-val",
     xlab='Num. Annotations', ylab='F-Score')
lines(y.rs.seg.full.mean, col='blue', lwd=2)



m.al.seg.full.improvement = m.al.seg.full - m.rs.seg.full
y.al.seg.full.improvement.mean = apply(X=m.al.seg.full.improvement, MARGIN=2, FUN=mean)
y.al.seg.full.improvement.sd = apply(X=m.al.seg.full.improvement, MARGIN=2, FUN=sd)
y.minus.sd = y.al.seg.full.improvement.mean - y.al.seg.full.improvement.sd
y.plus.sd = y.al.seg.full.improvement.mean + y.al.seg.full.improvement.sd
ylim = c(min(y.minus.sd), max(y.plus.sd))
plot(y.al.seg.full.improvement.mean, ylim=ylim, type='l', lwd=2, col='red', main='Improvement; Full; Seg-based x-val')
lines(y.minus.sd, col='red', lty='dotted')
lines(y.plus.sd, col='red', lty='dotted')
lines(x=1:length(y.minus.sd), y=rep(0, length(y.minus.sd)), col='black')
...
y.al.full.mean = apply(X=m.al.full, MARGIN=2, FUN=mean)
y.rs.full.mean = apply(X=m.rs.full, MARGIN=2, FUN=mean)
plot(y.al.full.mean, type='l', lwd=2, ylim=c(0, 1), col='red', main="Full; Mean; Frame-based when annotating segments")
lines(y.rs.full.mean, col='blue', lwd=2)
...
y.plus.sd = y.al.full.improvement.mean + y.al.full.improvement.sd
ylim = c(min(y.minus.sd), max(y.plus.sd))
plot(y.al.full.improvement.mean, ylim=ylim, type='l', lwd=2, col='red', main='Improvement; Full; Frame-based when annotating segments')
lines(y.minus.sd, col='red', lty='dotted')
lines(y.plus.sd, col='red', lty='dotted')
lines(x=1:length(y.minus.sd), y=rep(0, length(y.minus.sd)), col='black')
