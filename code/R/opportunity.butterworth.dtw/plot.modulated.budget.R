...
PlotALVersusRS = function(subject.number, al.file.name, rs.file.name) {
  al.file = paste('~/R/sf_ALTLAR/output/opportunity.butterworth.dtw.modulated/subject_', subject.number, 
                  '/output/modulated/l.subject.', subject.number, '.', al.file.name, sep='')
  load(file=al.file)
  al.data = l
  al.perf = GetAvgPerformance(l=al.data, perf.function=perf.function)
  
  rs.file = paste('~/R/sf_ALTLAR/output/opportunity.butterworth.dtw.modulated/subject_', subject.number, 
                  '/output/modulated/l.subject.', subject.number, '.', rs.file.name, sep='')
  load(file=rs.file)
  rs.data = l
...
al.file.name = 'k.3.exp.gamma.lambda.3.Rdata'
rs.file.name = 'k.3.exp.random.lambda.3.Rdata'
i.bubbles = c(190, 170, 95, 150)
