...
#
#
root.path = 'output/usc.had.seg.al/imperfect.seg/usc.had'
#root.path = 'output/usc.had.seg.al/imperfect.seg/pamap.2.protocol'
...
#subjects.features = obj
#subjects.features = subjects.features[-2] # contains NA
...
threshold = 0.2
th.dir = paste('threshold', threshold, sep='_')

online.confusion.metric = Online.FixedRate
offline.confusion.metric = Offline.Confidence
abs.path = paste(root.path, 'varying.f.mode', th.dir, sep='/')

cat(' Segmentation with f...\n')
for (f in fs) {
...
#stop('Stopping after gamma simulation. Nothing to worry about\n')
...
offline.confusion.metric = Offline.Confidence
#offline.confusion.metric = Offline.Margin
abs.path = paste(root.path, 'varying.gamma.min', th.dir, sep='/')

cat(' Segmentation with gamma...\n')
for (gamma in gammas) {
  this.dir = paste('gamma', gamma, sep='_')
  path = paste(abs.path, this.dir, sep='/')
  dir.create(path=path, recursive=T)
...
for (i in seq_along(model.builders)) {
  ...
  output.dir = paste('output/usc.had.seg.al/pamap.indoor', mb$short.name,
                     sep='/')
  graphics.dir = paste('graphics/usc.had.seg.al/pamap.indoor', mb$short.name,
                       sep='/')
